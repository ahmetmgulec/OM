...
const login_email = spacetimedb.reducer({
	...
	if (!credential.identity.isEqual(ctx.sender)) {
		...
		if (existing) ctx.db.replacedIdentity.oldIdentity.update({
			...existing,
		...
		for (const m of originalChannelMembers) ctx.db.channelMember.delete(m);
		for (const rm of originalRoleMembers) ctx.db.roleMember.delete(rm);
		ctx.db.emailCredential.email.update({
			...credential,
			identity: ctx.sender
		});
	}
