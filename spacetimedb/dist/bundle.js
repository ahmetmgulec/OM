...
const spacetimedb = schema({
	...
	roleMember: table({
		...
		indexes: [{
			...
			accessor: "role_member_by_user_id",
			name: "role_member_user_id",
			algorithm: "btree",
			columns: ["userId"]
		}]
	}, {
...
function hasGlobalAdmin(ctx) {
	for (const rm of ctx.db.roleMember.role_member_by_user_id.filter(ctx.sender)) {
		const r = ctx.db.role.id.find(rm.roleId);
		if (r && r.channelId === void 0 && (r.permissions & Permissions.ADMIN) !== 0n) return true;
	}
	return false;
}
function upsertReportedIp(ctx, identity, ip) {
	const existing = ctx.db.userReportedIp.identity.find(identity);
	if (existing) ctx.db.userReportedIp.identity.update({
		...existing,
		ip
	});
	else ctx.db.userReportedIp.insert({
		identity,
		ip
	});
}
/** Admins see all rows; everyone else gets an empty result (no IP leakage). */
const user_reported_ip_for_admin = spacetimedb.view({
	name: "user_reported_ip_for_admin",
	public: true
}, t.array(t.object("UserReportedIpRow", {
	identity: t.identity(),
	ip: t.string()
})), (ctx) => {
	if (!hasGlobalAdmin(ctx)) return [];
...
function looksLikePublicIp(addr) {
	if (addr.length > 45) return false;
	if (/^(\d{1,3}\.){3}\d{1,3}$/.test(addr)) return addr.split(".").map((p) => Number(p)).every((n) => n >= 0 && n <= 255);
	if (addr.includes(":")) return /^[0-9a-fA-F:.]+$/.test(addr) && addr.split(":").length >= 2;
	return false;
}
const report_client_ip = spacetimedb.reducer({ ip: t.string() }, (ctx, { ip }) => {
	const addr = ip.trim();
	if (!addr || !looksLikePublicIp(addr)) throw new SenderError("Invalid IP address");
	if (!ctx.db.user.identity.find(ctx.sender)) throw new SenderError("User not found");
...
const onConnect = spacetimedb.clientConnected((ctx) => {
	...
	if (user) {
		...
		if (connectionAddress && looksLikePublicIp(connectionAddress)) upsertReportedIp(ctx, ctx.sender, connectionAddress);
		...
		if (jwt && jwt.issuer === "https://auth.spacetimedb.com/oidc") {
			...
			if (authenticatedUsers.length === 1 && !hasAdminRole) try {
				...
				console.error("CRITICAL: First-user admin bootstrap failed — assign Admin manually in the dashboard or DB:", err);
			}
